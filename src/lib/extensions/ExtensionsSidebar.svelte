<script lang="ts">
  ...
  import { ArrowCounterClockwise, Check, DownloadSimple, Package, Power, Trash, X } from "phosphor-svelte";
  ...
  async function discard(extension: InstalledExtension) {
    if (!extension.previewHash) return;
    if (session.hasExternalPreview) discardCanvasPreview();
    working = true; error = "";
  ...
  function showCanvasPreview(label: string) { canvasPreview = label; }
  function applyCanvasPreview() { design.commitPreview(); canvasPreview = ""; }
  function discardCanvasPreview() { design.discardPreview(); canvasPreview = ""; }
  function close() { if (session.hasExternalPreview) discardCanvasPreview(); onClose(); }
</script>
...
<aside class="extensions-sidebar" data-extensions-sidebar>
  ...
  {#if canvasPreview}
    <div class="canvas-preview"><div><strong>Canvas preview</strong><span>{canvasPreview}</span></div><button class="discard" onclick={discardCanvasPreview}>Discard</button><button class="apply" onclick={applyCanvasPreview}><Check size={13} />Apply</button></div>
  {/if}
  ...
  <div class="body">
    ...
    {:else if tab === "panels"}
      {#if visible.length === 0}
        <div class="empty"><Package size={28} /><strong>No extension panels</strong><p>Import an extension to add custom controls and canvas actions.</p><button onclick={importExtension} disabled={working}><DownloadSimple size={14} />Import extension</button></div>
      {:else}
        {#each visible as item (item.extension.id + (item.extension.previewHash ?? item.extension.activeHash))}
          ...
          {#each item.manifest.contributes.sidebar as panel (panel.id)}
            <ExtensionPanel manifest={item.manifest} {panel} service={design} trial={item.trial} selectionCount={session.selectedIds.length} onCanvasPreview={showCanvasPreview} />
          {/each}
        {/each}
      {/if}
    {:else}
...
<style>
  .extensions-sidebar { position: absolute; z-index: 30; inset: 0 0 0 auto; width: var(--right-panel-width,241px); min-width: 0; display: flex; flex-direction: column; background: #242426; border-left: 1px solid #3b3b3f; color: #e8e8ea; }
  ...
  .trial,.canvas-preview { margin: 8px; padding: 9px; display: grid; grid-template-columns: 1fr auto auto; align-items: center; gap: 7px; border: 1px solid #665692; border-radius: 7px; background: #332d42; }.trial div,.canvas-preview div { min-width: 0; display: grid; gap: 2px; }.trial strong,.canvas-preview strong { font-size: var(--text-control); }.trial span,.canvas-preview span { color: #aaa0c5; font-size: var(--text-caption); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }.trial button,.canvas-preview button { height: 27px; padding: 0 8px; border: 1px solid #5a536c; border-radius: 5px; background: #40394f; color: #ded9ea; cursor: pointer; font-size: var(--text-small); }.trial .keep,.canvas-preview .apply { border-color: #238bd2; background: #1176bb; color: white; display: flex; align-items: center; gap: 4px; }
  .canvas-preview { margin-bottom: 0; border-color: #3e7195; background: #293641; }.canvas-preview span { color: #a8c3d7; }.banner { margin: 8px 8px 0; padding: 8px; border-radius: 6px; display: flex; gap: 7px; align-items: flex-start; justify-content: space-between; background: #4a292b; color: #fecaca; font-size: var(--text-small); }.banner button { border: 0; background: transparent; color: inherit; cursor: pointer; }
