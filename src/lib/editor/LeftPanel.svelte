<script lang="ts">
  ...
  const flatLayers = $derived.by(() => {
    ...
    [...session.document.rootIds].reverse().forEach((id) => visit(id, 0));
    return rows;
  });
...
<aside class="left-shell">
  ...
  <section class="panel">
    {#if session.leftTab === "file"}
      ...
      <div class="layers" role="tree" use:measureLayers onscroll={(event) => (layerScrollTop = event.currentTarget.scrollTop)}>
        ...
        {:else}<div class="layers-empty"><Layers3 size={22} /><p>No layers yet</p><span>Choose a tool and draw on the canvas.</span></div>{/if}
      </div>
    {:else}
